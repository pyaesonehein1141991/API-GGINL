...
public class LifeProposalService {
	...
	private List<AgentCommission> convertGroupFarmerToAgentCommission(GroupFarmerProposal groupFarmerProposal) {
		...
		try {
			...
			agentCommissionList
					.add(new AgentCommission(groupFarmerProposal.getId(), PolicyReferenceType.GROUP_FARMER_PROPOSAL,
							groupFarmerProposal.getAgent(), firstAgentCommission, new Date(), payment.getReceiptNo(),
							groupFarmerProposal.getPremium(), commissionPercent, AgentCommissionEntryType.UNDERWRITING,
							rate, (rate * firstAgentCommission), "KYT", (rate * groupFarmerProposal.getPremium())));
		}
		catch (Exception e) {
			e.printStackTrace();
		}
			...
			for (LifePolicy lifePolicy : studentLifePolicyList) {
				...
				if (lifePolicy.getPaymentChannel().equals(PaymentChannel.CHEQUE)
					...
					String customerId = lifePolicy.getCustomer() == null ? lifePolicy.getOrganization().getId()
							: lifePolicy.getCustomer().getId();
					TLF tlf3 = addNewTLF_For_PremiumDebitForRCVAndCHQ(payment, customerId, lifePolicy.getBranch(),
				...
				if (lifePolicy.getAgent() != null) {
					...
					AgentCommission ac = new AgentCommission(lifePolicy.getId(),
	...
	private List<AgentCommission> convertStudentLifePolicyToAgentCommission(List<LifePolicy> studentlifePolicyList) {
		...
		try {
			...
			studentlifePolicyList.forEach(lifePolicy -> {
				...
				double rate = payment.getRate();
				double firstAgentCommission = lifePolicy.getAgentCommission();
				agentCommissionList.add(new AgentCommission(lifePolicy.getId(), PolicyReferenceType.STUDENT_LIFE_POLICY,
